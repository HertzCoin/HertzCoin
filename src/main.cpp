...
static const int MODIFIER_INTERVAL_SWITCH = 2000; // pos start block

int64 nChainStartTime = 1407867569;
...
const string strMessageMagic = "HertzCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if (nHeight == 1)
        nSubsidy = 19749 * COIN; // Premine: 0.1% (19,749 HZ)
    else if (nHeight <= 30)
        nSubsidy = 1 * COIN; // first 30 blocks are 1 coin reward
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in HertzCoin
    bool fStrictPayToScriptHash = true; // Always active in HertzCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HertzCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "12/8";
        CTransaction txNew;
        txNew.nTime = nChainStartTime;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 1211014 << CBigNum(8888) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        assert(block.hashMerkleRoot == uint256("0x3a93a92f1f547721912982ba19d2e5bc2e14fa07c7244b09cc9941ac2892c8bc"));
...
unsigned char pchMessageStart[4] = { 0xe9, 0xf0, 0xaa, 0xb1 };
